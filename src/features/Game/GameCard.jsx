...
import { formatDateTimeObj, formatDate } from "../../utils/custom";
...
const GameCard = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.type === "scheduled") {
      const datetime = formatDate(props.game.Info?.DateOfMatch, "datetime");
  ...
  const getDescription = () => {
    ...
    else if (props.game.Header?.MatchTimeExtended)
      return props.game.Header?.MatchTimeExtended;
  ...
  const gotoEvent = () => {
    const sportName =
      props.game.Info?.SportName?.International.toLowerCase().replace(
        / /g,
        "-"
      );
      ...
      <p className={classes.LeftComponent}>
        {props.game.Header?.Status > 0 || props.type === "live" ? (
          ...
          <>
            ...
            <span className={classes.Time}>{datetime}</span>
          </>
        )}
      </p>
      ...
      <div className={classes.TeamContainer}>
        <div className={classes.TeamHome}>
          <div className={classes.TeamLogoWrapper}>
            <TeamLogo
              teamId={props.game.Info?.HomeTeamId}
              isHome={true}
              sportName={props.game.Info?.SportName?.International}
            />
          </div>
          <div className={classes.TeamName}>
            {translateNameWithLang(props.game.Info?.HomeTeamName)}
          </div>
        </div>
        ...
        {props.game.Info.AwayTeamName && (
          <div className={classes.TeamAway}>
            <div className={classes.TeamLogoWrapper}>
              <TeamLogo
                teamId={props.game.Info?.AwayTeamId}
                isHome={false}
                sportName={props.game.Info?.SportName?.International}
              />
            </div>
            <div className={classes.TeamName}>
              {translateNameWithLang(props.game.Info?.AwayTeamName)}
            </div>
          </div>
        )}
      </div>
