...
import {
  ...
  getUpdatedHeaders,
  getEventsToAdd,
  getEventToAddFromHeader,
  getEventToAddFromMarkets,
...
const LiveLoader = () => {
  ...
  const liveConnectionStored = useSelector(
    (state) => state.live.liveConnection
  );
  const incompleteDataEvents = useSelector(
    (state) => state.live.incompleteDataEvents
  );
  ...
  const connectToWs = (address) => {
      ...
      if (updateObj.length) {
        updateObj.forEach((updateItem) => {
          ...
          if (foundEvent) {
            ...
            const processedEvent = getEventToAddFromMarkets(
              updatedMarkets,
    ...
    liveConnection.on("onProgramUpdates", (message) => {
      ...
      if (updateObj.A && updateObj.A.length) {
        const eventsToAdd = getEventsToAdd(
          updateObj.A,
      ...
      if (updateObj.U) {
        ...
        Object.keys(updatedHeaders).forEach((key) => {
          ...
          dispatch(
            liveActions.updateHeadersProps({
              matchId: matchId,
    ...
    liveConnection.on("onHeadersList", (message) => {
      ...
      updateObj.forEach((headerItem) => {
        ...
        if (liveStateRef.current[matchId]) {
          ...
          const processedEvent = getEventToAddFromHeader(
            headerItem,
            incompleteDataRef.current
          );
          if (processedEvent.toAdd)
