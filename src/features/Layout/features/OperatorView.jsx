...
const OperatorView = () => {
  ...
  const placingBetLoading = useSelector(
    (state) => state.betslip.placingBetLoading
  );

  const [isVisible, setIsVisible] = useState(true);
  const [selectedStake, setSelectedStake] = useState(null); // Track selected stake button
  ...
  const betButton = useMemo(() => {
    if (
      ...
      slips?.length === 0 ||
      !selectedAccount ||
      selectedAccount.RoleId !== 40
    )
      return (
        <button className={classes.printTicket} disabled>
  ...
  const handleStakeSelect = (stakeValue) => {
    if (slips?.length > 0) {
    ...
    <div className={classes.operatorView}>
      <div className={classes.operatorViewTitle}>
        ...
        <span
          className={`${
            isVisible
              ? [classes.expandArrow, classes.Active].join(" ")
              : classes.expandArrow
          }`}
        >
      ...
      <div
        className={`${classes.loadTicketContainer} ${
          isVisible ? classes.slideDown : classes.slideUp
        }`}
      >
        <div>
          <div className={classes.accountSelectWidget}>{<AccountSelect />}</div>
          <LoadBooked />
          {betButton}
        </div>
        {
              ...
              {[1, 5, 10, 50, 100, 500, 1000, 1500, 2000, 3000].map((value) => (
                <div className={classes.favInputTypeButton} key={value}>
                  <input
                    ...
                    value={value}
                    style={
                      slips?.length > 0 && selectedStake === value
                        ? {
                            background: "#b3d3dd",
                            color: "black",
                            fontWeight: "bold",
