...
const Betslip = memo(function (props) {
  ...
  const savingBetLoading = useSelector(
    (state) => state.betslip.savingBetLoading
  );
  ...
  const betButton = useMemo(() => {
    if (!user)
      ...
      return (
        <button
          className={classes.BetButton}
          onClick={() => dispatch(betslipActions.acceptChanges())}
        >
        ...
        >
          {placingBetLoading ? (
            ...
            <span>
              ...
              {addThousandsSeparator(betslip?.totalStake, 2)}
            </span>
          )}
        </button>
      );
  ...
  const bonusButton = useMemo(() => {
    ...
    if (
      ...
      totalTemp > 0 &&
      bonusBalance &&
      bonusBalance >= totalTemp
    ) {
      return (
        <div
          className={classNames(classes.BonusButton, {
            [classes.selected]: isBonus,
          })}
        >
