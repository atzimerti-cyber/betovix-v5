...
const MyBetDetails = memo(function (props) {
  ...
  const getNotLiveGameScore = (ticketEvent) => {
    if (
      ticketEvent.Tournament.includes("Outright") ||
      ticketEvent.Tournament.includes("Specials")
    ) {
      return "";
    }
    ...
    const result =
      ticketEvent?.Result &&
      ticketEvent?.Result !== "-" &&
      JSON.parse(ticketEvent?.Result);

    if (result) {
      const score =
        result.away !== null ? result.home + "-" + result.away : result.home;
      return score;
    }
                  ...
                  {liveState[ticketEvent.EventId] && (
                    <>
                      ...
                      <div className={classes.Time}>
                        {getStatusText(liveState[ticketEvent.EventId])}
                      </div>
                    </>
                  )}
                </div>
                <div className={classes.InfoScore}>
                  {liveState[ticketEvent.EventId].Header?.Score}
                </div>
              </div>
            ) : (
              <div className={classes.InfoContainer}>
