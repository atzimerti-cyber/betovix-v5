...
const MyBet = (props) => {
  ...
  const getTotalOdds = () => {
    ...
    if (props.item.TicketEvents.length > 1)
      totalOdds = props.item.TicketEvents.reduce(
        (acc, cur) => acc * cur.Odd,
        1
      );
  ...
  useEffect(() => {
    if (cashedOutResult[props.item.TicketId] === "success") {
      setTimeout(() => {
        ...
        dispatch(
          getTicketCashouts(cashoutType, props.page, axiosController.signal)
        );
      ...
      >
        <div className={classes.Title}>
          ...
          <span className={classes.Stake}>
            {addThousandsSeparator(props.item.Stake)}
          </span>
          <span className={classes.Type}>
            {" "}
            {props.item.Type === "Singles"
              ? translate("Single")
              : translate("Multi")}{" "}
          </span>
        ...
        <div className={classes.DetailsContainer}>
          <section className={classes.CashoutSection}>
            ...
            >
              ...
              {cashedOutResult[props.item.TicketId] &&
                cashedOutResult[props.item.TicketId] === "success" &&
                ticketCashouts[props.item.TicketId] && (
                  <div className={classes.CashoutSuccess}>
                    {translate("Cashout Successful")}{" "}
                    {addThousandsSeparator(
                      ticketCashouts[props.item.TicketId].Metrics?.Cashout
                    )}
                    <SuccessIcon />
                ...
                (props.item.IsBonus ? (
                  <div className={classes.DisableWrapper}>
                    <div className={classes.DisabledButton}>
                      {translate("Cashout Disabled")}
                      <DisabledIcon />
                      <span className={classes.DisableReason}>
                        {translate(
                          "Can not cashout when ticket is placed with bonus"
                        )}
                      </span>
                    </div>
                  </div>
                    ...
                    {!ticketCashouts[props.item.TicketId].Metrics?.Cashout ? (
                      <div className={classes.CashoutSuspendedWrapper}>
                        <div className={classes.CashoutSuspended}>
                          ...
                          <Warning4Icon />
                          <span className={classes.BreakReason}>
                            {ticketCashouts[props.item.TicketId]?.BreakReason}
                          </span>
                        </div>
                      </div>
                    ) : (
