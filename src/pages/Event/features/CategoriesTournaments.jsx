  ...
  return (
    ...
    allTournaments.map((tournament, ctIndex) => {
      return (
        <div
          key={tournament.Id}
          data-category={`Category:${tournament.CategoryId}`}
          data-tournament={`Tournament:${tournament.Id}`}
        >
          <Accordion2
            icon={props.sport.icon}
            title={`${translateNameWithLang(
              tournament.CategoryName
            )} / ${translateNameWithLang(tournament.Name)}`}
            initOpen={ctIndex < 1}
            catIcon={`url('/flags/flags/${tournament?.CategoryName?.International}.png')`}
          >
            <Tournament
              tournament={tournament}
              slice="sportsbook"
              includePregame={false}
              includeLive={true}
              typeList
            />
          </Accordion2>
