...
const Leaderboard = () => {
  ...
  const loadingLeaderboard = useSelector(
    (state) => state.leaderboard.loadingLeaderboard
  );
    ...
    <div className={classes.PageContent}>
      <div className={classes.Banner}>
        <h1 className={classes.BannerTitle}>
          <span>{translate("TOP 10 LAST WINS")}</span>{" "}
        ...
        <div className={classes.TabPanel}>
          <table className={classes.LeaderboardTable}>
            ...
            {leaderboard ? (
              <tbody>
                {selectedTab === "Daily" &&
                  leaderboard?.standings?.daily?.standings?.map(
                    (standing, index) => {
                      let reward = null;

                      return (
                        <LeaderboardRow
                          key={standing.ticket.id}
                          standing={standing}
                          position={index + 1}
                          reward={reward}
                        />
              ...
              <tbody>
                {Array.from({ length: 10 }, (_, index) => (
                  <tr key={index} className={classes.LoadingPlaceholder}>
                    <td colSpan={4}>
                      <LoaderPlaceholder
                        extraStyles={{ borderRadius: "0.375rem" }}
                      />
                    </td>
                  </tr>
                ))}
