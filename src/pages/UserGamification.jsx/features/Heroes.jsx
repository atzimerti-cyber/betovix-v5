  ...
  const renderSelectButton = (canSelect, selectedHero) => {
    ...
    if (selectedHero)
      return (
        <MainButton disabled>
          {" "}
          <span>{translate("You have selected your hero")}</span>{" "}
        </MainButton>
      );
    else if (!selectedHero && !canSelect)
      return (
        <MainButton disabled>
          {" "}
            ...
            <div className={classes.rightCol}>
              <div className={classes.heroName}>
                <p className={classes.DescTitle}>
                  {translate(
                    displayedHero.metadata.HeroName +
                      " " +
                      displayedHero.metadata.HeroSubName
                  )}
                </p>
              </div>
              <div className={classes.heroText}>
                <p className={classes.Description}>
                  {displayedHero?.description?.replace(/<\/?p>/g, "")}
                </p>
              </div>

              <Levels
                slidesPerView={slidesPerView}
                displayedHero={displayedHero}
                activeLevel={activeLevel}
                onChangeLevel={(level) => setActiveLevel(level)}
                profile={true}
              />
              <Milestones
