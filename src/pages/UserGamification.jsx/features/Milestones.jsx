...
const Milestones = (props) => {
  ...
  const currentUserLevel = useSelector(
    (state) => state.gamification.currentLevel
  );
  ...
  useEffect(() => {
    ...
    const foundIndex = heroLevels.findIndex(
      (l) => l.id === props.activeLevel?.id
    );
    if (foundIndex > -1) {
                    ...
                    <div className={classes.DiamondContainer}>
                      {heroLevels && heroLevels.length > 0 ? (
                        <>
                          {[
                            {
                              id: `${heroLevels[thisLevelIndex]}placeholder`,
                              milestone: "before_first",
                            },
                            ...
                            <LevelDiamond
                              key={milestone.id}
                              complete={
                                (index == 0 || milestone?.progress == 100) &&
                                true
                              }
                              index={index}
                            />
                    ...
                    <>
                      {thisLevelIndex < heroLevels.length && (
                        <MilestoneCard
                          ...
                          complete={
                            heroLevels[thisLevelIndex]?.id ===
                              currentUserLevel?.id ||
                            heroLevels[thisLevelIndex]?.completed ||
                            !user ||
                            !selectedHero ||
                            props.profile
                          }
                      ...
                      {heroLevels[thisLevelIndex]?.milestones.map(
                        (milestone, index) => (
                          <MilestoneCard
