...
export const getHeroes = (signal) => {
  return async (dispatch, getState) => {
    try {
      ...
      if (
        Object.keys(state.gamification.displayedHero).length == 0 &&
        heroes.length > 0
      ) {
...
export const BuyHeroLevel = (hero, level, signal) => {
  return async (dispatch) => {
    try {
      const response = await axiosApi.get(
        `/ModuleGamification/BuyHeroLevel?heroId=${hero}&levelId=${level}`,
        {
          signal: signal,
          baseURLOverride: config.VITE_GAMIFICATION_STORETUBE,
        }
  ...
  return async (dispatch) => {
    try {
      ...
      const selectedHero = {
        ...
        description:
          response.data.Contents.Hero.Achievement?.TermsAndConditions,
...
export const claimReward = (Id) => {
  return async (dispatch) => {
    try {
      ...
      if (response.status !== 200 || response.data.Status.StatusCode !== 200)
        throw toast.error(translate(response.data.Contents));
...
export const heroProgress = () => {
  return async (dispatch) => {
    try {
      ...
      const progressFixed = !isNaN(parseFloat(progress))
