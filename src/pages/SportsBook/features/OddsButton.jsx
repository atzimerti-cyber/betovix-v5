...
const OddsButton = (props) => {
  ...
  const allowedToAdd = (newSlip) => {
    ...
    if (
      betType !== "Single" &&
      (betType === "System" || hasAnEventMoreThanOnePoint(newSlip))
    ) {
      if (allowed > ticketSettings.TicketSettings.MAX_ALLOWED_SYSTEM_POINTS)
        allowed = ticketSettings.TicketSettings.MAX_ALLOWED_SYSTEM_POINTS;
    }
  ...
  useEffect(() => {
    ...
    if (foundMarket && foundMarket.MarketFields) {
      const foundMarketField = foundMarket.MarketFields.find(
        (f) => f.FieldId === props.marketField.FieldId
      );
